...
Key.escape_hold_time = 0;
Key.escape_hold_start = 0;
Key.escape_hold_required = 0.5;
...
Key.Event = function(key, down)
{
        ...
        if (key === Key.k.escape)
        {
                if (down === true)
                {
                        Key.escape_hold_start = Sys.FloatTime();
                        Key.escape_hold_time = 0;
                }
                else
                {
                        Key.escape_hold_time = Sys.FloatTime() - Key.escape_hold_start;
                        if (Key.escape_hold_time >= Key.escape_hold_required)
                        {
