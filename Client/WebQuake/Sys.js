...
Sys.Error = function(text)
{
        ...
        // Don't show alert for missing game files, just log the error
        if (text.indexOf('couldn\'t load') === -1 && text.indexOf('not found') === -1) {
                alert(text);
        }
        console.error(text);
...
// Check if game data files exist before initializing the full game
Sys.CheckGameData = function() {
...
// Initialize the full game only when data files are available
Sys.InitFullGame = function() {
...
{
        // Check if game data exists, only initialize full game if it does
        Sys.CheckGameData().then(hasGameData => {
                if (hasGameData) {
                        // Game data exists, initialize the full game
                        Sys.InitFullGame();
                } else {
                        // No game data, keep the download interface active
                        console.log('Game data not found, showing download interface');
                        // Keep the download button and interface visible
                }
        });
};
